...
随机数生成是计算机科学中的一个重要概念，用于生成看似随机的数字序列。在NumPy中，随机数生成功能非常强大，提供了多种分布的随机数生成方法，以及各种随机采样和排列功能。
...
NumPy提供了强大的随机数生成功能，支持多种分布的随机数生成、随机采样和排列。使用新的随机数生成器 `default_rng()` 可以获得更好的性能和可重复性。

### 10.1 核心功能

- **多种分布**：均匀分布、正态分布、二项分布等
- **随机采样**：有放回和无放回采样，加权采样
- **随机排列**：打乱数组，生成随机排列
- **可重复性**：通过设置种子保证结果可重复
- **高性能**：底层实现高效，适合生成大型数组
