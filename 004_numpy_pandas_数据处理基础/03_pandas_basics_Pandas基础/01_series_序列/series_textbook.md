...
1. 序列的基本数据结构是________________。
2. 从字典创建序列时，字典的键会成为序列的________________。
...
1. 创建一个包含10个随机整数的序列，并计算其均值、中位数和标准差。
2. 创建一个从2023年1月1日开始的30天时间序列，值为随机数，然后计算7天移动平均。
3. 创建一个字符串序列，将所有字符串转换为大写，并统计每个字符串的长度。
4. 创建一个包含缺失值的序列，使用不同的方法处理缺失值。

## 第十三章：总结

### 13.1 知识回顾

1. **序列的创建**：从列表、NumPy数组、字典创建序列
2. **元素访问**：通过索引、切片、布尔索引访问元素
3. **属性和方法**：使用各种属性和方法处理数据
4. **运算**：基本运算、序列之间的运算、索引对齐
5. **字符串方法**：处理字符串数据
6. **时间序列**：创建和分析时间序列数据
7. **性能优化**：优化内存使用和计算速度
8. **应用示例**：数据统计、过滤、转换、时间序列分析
